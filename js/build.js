...
function renderBuildings() {
    ...
    list.innerHTML = BUILD_DATA.map(function(b) {
        ...
        return '<div class="card"><div><div style="font-weight:bold">' + b.name + ' Lv.' + my.level + '</div><div style="font-size:9px;color:#888">产出 ' + Math.floor(b.base*my.level) + '/h</div></div><button class="btn btn-small" onclick="upgradeBuild(' + b.id + ')">' + cost + '💰</button></div>';
...
function upgradeBuild(id) {
    ...
    state.lingqi -= cost;
    my.level++;
    save(); renderBuildings(); renderAll();
    showToast(b.name + " -> Lv." + my.level);
}
